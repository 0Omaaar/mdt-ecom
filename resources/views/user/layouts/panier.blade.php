...
<div class="sidebar-menu-wrapper">
    <div class="cart_sidebar">
        <button type="button" class="close_btn"><i class="fal fa-times"
                style="background-color: rgb(255, 255, 255)"></i></button>
                    ...
                    <li>
                        <div class="item_image">
                            @if ($item->product->dolibarr_id == null)
                                <img src="{{ asset('images/products/' . $item->product->id . '/' . $item->product->image) }}"
                                    alt="image_non_trouvée">
                            @else
                        ...
                        <div class="item_content">
                            ...
                            <span class="item_price">{{ $item->product->price }} DHS x
                                <b>{{ $item->quantity }}</b></span>
                        </div>
                        <form action="{{ route('removeItemFromCart', ['id' => $item->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="remove_btn" style="background-color: white"
                                title="Retirer Produit du Panier"><i class="fal fa-trash-alt"
