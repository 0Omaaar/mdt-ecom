                                ...
                                <tr>
                                        ...
                                        <div class="cart_product">
                                            ...
                                            <div class="item_content">
                                                <h4 class="item_title"><a style="color: #333e48"
                                                        href="{{ route('product', ['id' => $item->product_id]) }}">{{ $item->product->name }}</a>
                                                </h4>
                                                <span class="item_type">{{ $item->product->category->name }}</span>
                                            </div>
                                            <form action="{{ route('removeItemFromCart', ['id' => $item->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="remove_btn"
                                                    title="Retirer Produit du Panier"><i
                                    ...
                                    <td>
                                        <div class="quantity_input">
                                            <span
                                                onclick="window.location.href='{{ route('cart.decreaseQuantity', ['id' => $item->id]) }}'">–</span>
                                            <input class="input_number" id="item_quantity" type="text"
                                                value="{{ $item->quantity }}">
                                            <span
                                                onclick="window.location.href='{{ route('cart.addQuantity', ['id' => $item->id]) }}'">+</span>
                                        </div>
                                    </td>
                                    <td><span class="total_price">{{ $item->quantity * $item->product->price }}
                                            DHS</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="row justify-content-lg-end mb-5">
                    <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
                        <div class="cart_pricing_table pt-0 text-uppercase" data-bg-color="#f2f3f5">
                            ...
                            <a href="{{ route('checkout.page') }}" class="custom_btn bg_success">Valider la
                                Commande</a>
                        </div>
                    </div>
                </div>
